  ...
  async captureAndRecognize() {
    ...
    setInterval(async () => {
      ...
      for (const detection of resizedDetections) {
        ...
        // ==========================================================
        //  SOLUÇÃO: Adicione esta verificação de segurança
        // ==========================================================
        if (detection && detection.detection && detection.detection.box) {
          ...
          // Garante que o gym_id existe antes de enviar
          if (!gymId) {
            console.error('ID da academia não encontrado. Abortando envio.');
          ...
          this.http.post(this.recognitionApiUrl, formData).subscribe({
            ...
            error: (err) => {
              // Não logar erros 404 como falhas críticas, pois é uma resposta esperada
              if (err.status !== 404) {
          ...
          break; // envia apenas o primeiro rosto detectado e válido
        }
      }
